...
void init_ADC() {
    ADCON1 = 0x80;  // ????? ADC
    ADCON0 = 0x09;  // ???? ???? ADC // A2 PIN ADC
}

unsigned int read_ADC() {
    ADCON0bits.GO_nDONE = 1;  // ???? ?????
    while (ADCON0bits.GO_nDONE);  // ?????? ???? ????? ?????
    return ((ADRESH << 8) + ADRESL);  // ?????? ????? ADC
}
...
void main(void)
{
  ...
  initialise();
    ...
    // TRISA = 0xFF;  // ????? ???? A ?? ????? ?????
    TRISCbits.TRISC0 = 0;  // ????? ???? C0 ?? ????? ????? (??? ??)
     TRISCbits.TRISC5 = 0;  // ????? ???? C0 ?? ????? ????? (??? ??)
    init_ADC();  // ???????? ????? ADC
  ...
  while(1)
    {
        ...
        unsigned int adc_value = read_ADC();  // ?????? ????? ADC
        float temperature = (adc_value * 5.0 / 1023.0) * 100;  // ????? ????? ADC ?? ???
